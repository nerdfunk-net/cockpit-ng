...
export function useDevicePreview(
  ...
  const [showPreviewResults, setShowPreviewResults] = useState(
  ...
  const treeToFlatConditions = useCallback(
    (tree: ConditionTree): LogicalCondition[] => {
  ...
  const buildOperationsFromTree = useCallback(
    (tree: ConditionTree | ConditionGroup): BackendOperation[] => {
      ...
      items.forEach(item => {
        ...
        if (
          'type' in item &&
          item.type === 'group' &&
          (item as ConditionGroup).logic === 'NOT'
        ) {
