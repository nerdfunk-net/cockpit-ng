...
export function useSnapshots() {
  ...
  const executeSnapshot = useCallback(
    async (request: SnapshotExecuteRequest) => {
      setLoading(true)
      setError(null)
      try {
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to execute snapshot'
        setError(message)
  ...
  const compareSnapshots = useCallback(
    async (request: SnapshotCompareRequest) => {
      setLoading(true)
      setError(null)
      try {
  ...
  const deleteSnapshotDbOnly = useCallback(
    async (snapshotId: number) => {
      setLoading(true)
      setError(null)
      try {
  ...
  const deleteSnapshotWithFiles = useCallback(
    async (snapshotId: number) => {
      setLoading(true)
      setError(null)
      try {
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to delete snapshot and files'
        setError(message)
