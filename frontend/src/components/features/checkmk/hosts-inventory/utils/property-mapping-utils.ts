...
export function initializePropertyMappings(
  ...
  const statusKey = Object.keys(attrs).find(
    key => key.toLowerCase() === 'status' || key === 'tag_status'
  )
  ...
  const roleKey = Object.keys(attrs).find(
    key => key.toLowerCase() === 'role' || key === 'tag_role'
  )
  ...
  const deviceTypeKey = Object.keys(attrs).find(
    key => key.toLowerCase() === 'device_type' || key === 'tag_device_type'
  )
  ...
  Object.keys(attrs).forEach(key => {
    if (key.startsWith('tag_')) {
      ...
      const isStatusAttribute =
        reverseAttr2htg[key] && reverseAttr2htg[key].startsWith('status')
...
export function resolveNautobotId(
  ...
  // Try partial match
  const partialMatch = metadataArray.find(item =>
    item.name.toLowerCase().includes(valueStr)
  )
