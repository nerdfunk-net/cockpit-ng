...
import {
  ...
  type InterfaceMappingData,
} from '../utils/property-mapping-utils'
...
import {
  ...
  parseInterfacesFromAddresses,
  type CheckMKInterface,
...
export function useNautobotSync({
  ...
  const [selectedHostForSync, setSelectedHostForSync] = useState<CheckMKHost | null>(
    null
  )
  const [nautobotDevice, setNautobotDevice] = useState<Record<string, unknown> | null>(
    null
  )
  const [checkingNautobot, setCheckingNautobot] = useState(false)

  // Nautobot data
  const [nautobotMetadata, setNautobotMetadata] = useState<NautobotMetadata | null>(
    null
  )
  const [propertyMappings, setPropertyMappings] =
    useState<Record<string, PropertyMapping>>(EMPTY_MAPPINGS)
  ...
  // IP address statuses
  const [ipAddressStatuses, setIpAddressStatuses] = useState<Array<{
    id: string
    name: string
  }> | null>(null)

  // IP address roles
  const [ipAddressRoles, setIpAddressRoles] = useState<Array<{
    id: string
    name: string
  }> | null>(null)
  ...
  const [interfaceMappings, setInterfaceMappings] = useState<
    Record<string, InterfaceMappingData>
  >({})
  ...
  const loadInventoryData = useCallback(
    async (hostName: string, source: InterfaceSource) => {
      try {
  ...
  const loadNautobotMetadata = useCallback(async () => {
    try {
      ...
      const [
        ...
        apiCall<{ results: Array<{ id: string; name: string }> }>(
          'nautobot/roles/devices'
        ),
        apiCall<{ results: Array<{ id: string; name: string }> }>(
          'nautobot/statuses/device'
        ),
        apiCall<Array<{ id: string; display: string; model: string }>>(
          'nautobot/device-types'
        ),
        apiCall<{ results: Array<{ id: string; name: string }> }>('nautobot/platforms'),
        apiCall<{ results: Array<{ id: string; name: string; key: string }> }>(
          'nautobot/custom-fields/devices'
        ),
        apiCall<{ results: Array<{ id: string; name: string }> }>(
          'nautobot/statuses/ipaddress'
        ),
        apiCall<{ results: Array<{ id: string; name: string }> }>(
          'nautobot/roles/ipaddress'
        ),
      ])

      // Handle different response formats: some endpoints return { results: [...] }, others return array directly
      const extractResults = <T>(
        response: { results?: T[] } | T[] | undefined
      ): T[] => {
        if (!response) return []
      ...
      const mappedDeviceTypes = extractResults(deviceTypes).map(
        (dt: { id: string; display?: string; model?: string; name?: string }) => ({
          id: dt.id,
   ...
   */
  const handleSyncToNautobot = useCallback(
    async (host: CheckMKHost) => {
      try {
        ...
        try {
          ...
          console.log(
            '[useNautobotSync] Search result for',
            host.host_name,
            ':',
            searchResult
          )
          ...
          if (searchResult?.devices && searchResult.devices.length > 0) {
            ...
            const deviceDetails = await apiCall<Record<string, unknown>>(
              `nautobot/devices/${deviceId}`
            )
   ...
   */
  const updatePropertyMapping = useCallback(
    (checkMkKey: string, nautobotField: string) => {
   ...
   */
  const executeSyncToNautobot = useCallback(
    async (formData: DeviceFormValues, deviceId?: string) => {
      ...
      try {
        ...
        if (existingDeviceId) {
          ...
          onMessage(
            `Successfully updated ${selectedHostForSync.host_name} in Nautobot`,
            'success'
          )
        } else {
          ...
          onMessage(
            `Successfully created ${selectedHostForSync.host_name} in Nautobot`,
            'success'
          )
          ...
          if (createResponse?.id) {
            try {
              const deviceDetails = await apiCall<Record<string, unknown>>(
                `nautobot/devices/${createResponse.id}`
              )
              setNautobotDevice(deviceDetails || null)
            } catch (err) {
