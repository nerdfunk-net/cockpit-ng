...
const backupTemplateSchema = baseTemplateSchema
  .extend({
...
const runCommandsTemplateSchema = baseTemplateSchema
  .extend({
...
const syncDevicesTemplateSchema = baseTemplateSchema
  .extend({
...
const compareDevicesTemplateSchema = baseTemplateSchema
  .extend({
...
const cacheDevicesTemplateSchema = baseTemplateSchema
  .extend({
...
// CSV Export job type schema
const csvExportTemplateSchema = baseTemplateSchema.extend({
  job_type: z.literal('csv_export'),
  csv_export_repo_id: z.number(),
  csv_export_file_path: z.string().min(1, 'File path is required').max(500),
  csv_export_properties: z
    .array(z.string())
    .min(1, 'At least one property must be selected'),
  csv_export_delimiter: z.string().max(10).optional(),
  csv_export_quote_char: z.string().max(10).optional(),
