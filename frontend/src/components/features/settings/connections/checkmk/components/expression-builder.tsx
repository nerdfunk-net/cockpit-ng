...
export function ExpressionBuilder({ value, onChange }: ExpressionBuilderProps) {
  ...
  const updateConnector = (index: number, operator: 'and' | 'or') => {
  ...
  const removeCondition = (conditionIndex: number) => {
    ...
    if (conditionIndex === 0) {
      ...
      onChange([...value.slice(0, flatIndex - 1), ...value.slice(flatIndex + 1)])
    }
  }
          ...
          <div key={flatIndex}>
            {idx > 0 && (
              <div className="flex items-center gap-2 my-1 ml-1">
                <Select
                  value={
                    connectors[idx - 1]?.type === 'connector'
                      ? (connectors[idx - 1] as { type: 'connector'; operator: string })
                          .operator
                      : 'and'
                  }
