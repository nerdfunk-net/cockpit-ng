...
export function useCredentialForm(options: UseCredentialFormOptions = DEFAULT_OPTIONS) {
  ...
  const defaultValues: CredentialFormValues = {
    ...
    type:
      (credential?.type as 'ssh' | 'ssh_key' | 'tacacs' | 'generic' | 'token') || 'ssh',
    password: '',
    ssh_private_key: '',
    ssh_passphrase: '',
  ...
  return useForm<CredentialFormValues | CredentialEditValues>({
    resolver: zodResolver(isEditing ? credentialEditSchema : credentialFormSchema),
    defaultValues,
    mode: 'onChange',
  })
}
