  ...
  const handleUpdate = useCallback(
    async (dryRun: boolean = false) => {
      ...
      try {
        ...
        const errorMsg =
          error instanceof Error ? error.message : 'Failed to submit update task'
        setParseError(errorMsg)
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="!max-w-[880px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file to update multiple devices in Nautobot. Configure the CSV
            format settings below, then upload a file with an identifier column (id,
            name, or ip_address) and the properties you want to update.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <Alert className="status-info">
                ...
                <AlertDescription className="text-sm">
                  <strong>Required:</strong> CSV must have at least one identifier
                  column: <code>id</code>, <code>name</code>, or{' '}
                  <code>primary_ip4</code>
                  <br />
                  <strong>Updates:</strong> All other columns will be used to update
                  device properties
                  <br />
          ...
          {parsedData && !showProgress && (
            <div className="space-y-4">
              <Alert className="status-success">
                ...
                <AlertDescription>
                  CSV parsed successfully! Found <strong>{parsedData.rowCount}</strong>{' '}
                  devices to update.
                </AlertDescription>
              </Alert>
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {parsedData.rows.slice(0, 3).map(row => {
                          const rowId = parsedData.identifierField
                            ? row[parsedData.identifierField]
                            : JSON.stringify(row)
                          return (
          ...
          {showProgress && taskId && (
            <div className="space-y-4">
              <Alert className="status-info">
                <Info className="h-4 w-4" />
                <AlertDescription>
                  Update task is running in the background. You can track progress in
                  the Jobs/View app.
                </AlertDescription>
              </Alert>
