...
export function useRackDevicesQuery({
  rackId,
}: UseRackDevicesQueryOptions = DEFAULT_OPTIONS) {
    ...
    queryFn: async (): Promise<RackDevice[]> => {
      ...
      const mappedDevices: RackDevice[] = devices.map(d => {
        ...
        const normalizedFace: 'front' | 'rear' | null =
          rawFace === 'front' ? 'front' : rawFace === 'rear' ? 'rear' : 'front' // default to front — most rack devices are front-mounted
        return {
          id: d.id,
          name: d.name,
          position: d.position,
      ...
      const reservations = Array.isArray(rack.rack_reservations)
        ? rack.rack_reservations
        : []
