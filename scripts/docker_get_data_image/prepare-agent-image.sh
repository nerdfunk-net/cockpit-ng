#!/bin/bash
# prepare-agent-image.sh - Build air-gap ready Cockpit Get Data Agent image
# Run this script on a machine with internet access, then transfer the artifact.

set -e

echo "Building Cockpit Get Data Agent Image for Air-Gap Deployment"
echo "============================================================="

IMAGE_NAME="cockpit-agent-get-data"
IMAGE_TAG="latest"
FULL_IMAGE_NAME="${IMAGE_NAME}:${IMAGE_TAG}"
ARTIFACT_DIR="scripts/docker_get_data_image/airgap-artifacts"
...
echo "  2. gunzip cockpit-agent-get-data.tar.gz"
echo "  3. docker load -i cockpit-agent-get-data.tar"
echo "  4. Copy scripts/docker_get_data_image/.env.example to .env and configure:"
echo "       REDIS_HOST, REDIS_PASSWORD, AGENT_ID, COCKPIT_SHARED_SECRET"
echo "  5. Copy and edit config.yaml (pipeline definition) for your targets"
echo "  6. cd scripts/docker_get_data_image && docker compose up -d"
echo ""
