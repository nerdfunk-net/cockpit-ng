#!/bin/bash
# prepare-agent-image.sh - Build air-gap ready Cockpit Ansible Agent image
# Run this script on a machine with internet access, then transfer the artifact.

set -e

echo "Building Cockpit Ansible Agent Image for Air-Gap Deployment"
echo "============================================================"

IMAGE_NAME="cockpit-agent-ansible"
IMAGE_TAG="latest"
FULL_IMAGE_NAME="${IMAGE_NAME}:${IMAGE_TAG}"
ARTIFACT_DIR="scripts/docker_ansible_image/airgap-artifacts"
...
echo "  2. gunzip cockpit-agent-ansible.tar.gz"
echo "  3. docker load -i cockpit-agent-ansible.tar"
echo "  4. docker run -d \\"
