#!/bin/bash
# prepare-agent-image.sh - Build air-gap ready Cockpit Ping Agent image
# Run this script on a machine with internet access, then transfer the artifact.

set -e

echo "Building Cockpit Ping Agent Image for Air-Gap Deployment"
echo "========================================================"

IMAGE_NAME="cockpit-agent-ping"
IMAGE_TAG="latest"
FULL_IMAGE_NAME="${IMAGE_NAME}:${IMAGE_TAG}"
ARTIFACT_DIR="scripts/docker_ping_image/airgap-artifacts"
...
echo "  2. gunzip cockpit-agent-ping.tar.gz"
echo "  3. docker load -i cockpit-agent-ping.tar"
echo "  4. Copy scripts/docker_ping_image/.env.example to .env and configure:"
echo "       REDIS_HOST, REDIS_PASSWORD, AGENT_ID, COCKPIT_SHARED_SECRET"
echo "  5. cd scripts/docker_ping_image && docker compose up -d"
echo ""
