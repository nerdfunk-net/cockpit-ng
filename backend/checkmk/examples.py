    ...
    try:
        ...
        if change_count > 0:
            ...
            if activation.get("redirected"):
                logger.info(
                    f"✓ Changes activated (redirected to: {activation.get('location')})"
                )
            else:
    ...
    try:
        ...
        try:
            services = client.get_host_services(
                hostname, columns=["description", "state"]
            )
            ...
            for service in services.get("value", [])[:3]:
                ...
                state_text = {0: "OK", 1: "WARNING", 2: "CRITICAL", 3: "UNKNOWN"}.get(
                    state, f"State {state}"
                )
    ...
    hosts_to_create = [
        {
            ...
            "folder": "/",
            "attributes": {"ipaddress": "192.168.1.101", "alias": "Bulk Test Server 1"},
        },
        {
            "host_name": "bulk-test-2",
            "folder": "/",
            "attributes": {"ipaddress": "192.168.1.102", "alias": "Bulk Test Server 2"},
        },
    ]
    ...
    try:
        ...
        try:
            new_group = client.create_host_group(
                name=test_group_name, alias="API Test Group"
            )
