        ...
        try:
            ...
            if host_count > 0:
                ...
                for i, host in enumerate(hosts.get('value', [])[:3]):
                    ...
                    logger.info(f"     {i+1}. {host_id} - IP: {ip_address}, Alias: {alias}")
        except CheckMKAPIError as e:
        ...
        try:
            ...
            if monitored_count > 0:
                ...
                for i, host in enumerate(monitored.get('value', [])[:3]):
                    ...
                    logger.info(f"     {i+1}. {name} - {state_text} ({address}) - Last check: {last_check}")
        except CheckMKAPIError as e:
    ...
    try:
        ...
        try:
            monitored_info = client.get_monitored_host(
                test_hostname,
                columns=['name', 'state', 'last_check', 'plugin_output']
            )
