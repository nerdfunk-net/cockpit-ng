...
def _execute_run_commands(
    ...
    try:
        ...
        if not template_content:
            logger.error(f"ERROR: Command template content not found for '{command_template_name}'")
            return {
                "success": False,
            ...
            for idx, device_id in enumerate(target_devices, 1):
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"[{idx}] ✗ Template rendering failed: {render_error}")
                    ...
                    if not commands:
                        logger.warning(f"[{idx}] ⚠ No commands to execute after rendering")
                    ...
                    if result["success"]:
                        ...
                        logger.info(f"[{idx}]   - Output length: {len(result['output'])} chars")
                        successful_devices.append(device_result)
                    else:
                        device_result["error"] = result.get("error", "Command execution failed")
                        logger.error(f"[{idx}] ✗ Command execution failed: {result.get('error')}")
                        failed_devices.append(device_result)
