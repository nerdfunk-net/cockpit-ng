    ...
    try:
        ...
        for schedule in schedules:
            try:
                ...
                if not template_id:
                    logger.warning(f"Schedule {schedule['id']} has no template_id, skipping")
                    continue
                
                template = job_template_manager.get_job_template(template_id)
                if not template:
                    logger.warning(f"Template {template_id} not found for schedule {schedule['id']}")
                    continue
        ...
        if dispatched:
            logger.info(f"Dispatched {len(dispatched)} jobs: {[d['job_name'] for d in dispatched]}")
...
def _get_target_devices(template: dict, job_parameters: Optional[dict] = None) -> Optional[list]:
    """
    ...
    if inventory_source == 'all':
        ...
        try:
            ...
            try:
                ...
                operations = _convert_conditions_to_operations(saved_inventory.conditions)
            ...
            try:
                ...
                try:
                    ...
                    if comparison_result:
                        completed_count += 1
                        # DeviceComparison has 'result' field: 'identical', 'different', 'host_not_found'
                        has_differences = comparison_result.result not in ['identical']
                        if has_differences:
                            differences_found += 1
                        ...
                        nb2cmk_db_service.add_device_result(
                            ...
                            checkmk_status=comparison_result.result,  # 'identical', 'different', 'host_not_found'
