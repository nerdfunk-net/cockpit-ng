...
def update_devices_task(
    ...
    try:
        ...
        logger.info("Number of devices: %s", len(devices))
        ...
        for idx, device_data in enumerate(devices, 1):
            if not isinstance(device_data, dict):
                logger.warning("Device %s is not a dict, skipping", idx)
                skipped.append(
                    {
                        "device_index": idx,
            ...
            try:
                ...
                if dry_run:
                    ...
                    if interfaces:
                        logger.debug("Interfaces: %s interface(s)", len(interfaces))
        ...
        logger.info("  - Total: %s", total_devices)
...
def _prepare_device_data(
    ...
    if interfaces and not isinstance(interfaces, list):
        logger.warning("interfaces field is not a list, ignoring: %s", type(interfaces))
        interfaces = None
