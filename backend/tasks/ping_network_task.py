...
def _fping_networks(
    ...
    try:
        ...
        logger.debug("Combined output length: %s bytes", len(all_output))

        if all_output:
            lines = all_output.strip().split("\n")
            logger.debug("Parsing %s total lines from fping output", len(lines))
...
def _resolve_dns(ip: str) -> str:
    ...
    try:
        ...
        logger.debug("DNS resolution failed for %s: %s", ip, e)
        return ""
    ...
    try:
        ...
        for idx, cidr in enumerate(cidrs):
            try:
                ...
                logger.error("Failed to expand CIDR %s: %s", cidr, e)
                network_ips[cidr] = []
        ...
        logger.error("Ping network task failed: %s", e, exc_info=True)
