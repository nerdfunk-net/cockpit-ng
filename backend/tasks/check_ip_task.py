    ...
    try:
        ...
        for row in csv_reader:
            if not first_row_processed:
                logger.info("Detected CSV columns: %s", list(row.keys()))
            ...
            if "ip_address" in row and "name" in row:
                ...
                logger.warning("Row missing required columns: %s", row)
        ...
        if nautobot_devices:
            logger.info("Sample Nautobot devices: %s", nautobot_devices[:3])
        ...
        for i, csv_device in enumerate(csv_devices):
            ...
            try:
                if csv_ip_clean in nautobot_lookup:
                    ...
                    logger.info("IP %s not found in Nautobot lookup", csv_ip_clean)
                    results.append(
                        {
                            "ip_address": csv_ip_clean,  # Use cleaned IP in results
                            "device_name": csv_name,
        ...
        logger.error("Error in check_ip_task: %s", str(e))
