...
def check_ip_task(self, csv_content: str, delimiter: str = None, quote_char: str = None) -> Dict[str, Any]:
    ...
    """
    logger.info(f"check_ip_task called with delimiter='{delimiter}', quote_char='{quote_char}'")
    try:
        # Use provided settings if they are valid (not None, not 'None', not empty)
        use_provided = (
            delimiter is not None and delimiter != 'None' and delimiter != '' and
            quote_char is not None and quote_char != 'None' and quote_char != ''
        )
        
        if use_provided:
            # Use provided parameters
            logger.info(f"Using provided CSV settings - delimiter: '{delimiter}', quote_char: '{quote_char}'")
        else:
            ...
            if not delimiter or not quote_char:
                ...
                logger.info(f"Using defaults - delimiter: '{delimiter}', quote_char: '{quote_char}'")
            
            logger.info(f"Raw settings from database: {settings}")
            logger.info(f"Using final CSV settings - delimiter: '{delimiter}', quote_char: '{quote_char}'")
        ...
        for row in csv_reader:
            if not first_row_processed:
                ...
                logger.info(f"Looking for required columns: 'ip_address' and 'name'")
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 0, 'total': len(csv_devices), 'message': 'Loading Nautobot devices...'}
        )
        ...
        while True:
            try:
                ...
                response_data = loop.run_until_complete(device_query_service.get_devices(
                    limit=limit,
                    offset=offset,
                    filter_type=None,
                    filter_value=None
                ...
                logger.error(f"Error loading devices from device_query_service at offset {offset}: {str(e)}")
                return {
                    'success': False,
                    'error': f'Failed to load devices from device service: {str(e)}'
                }
        
        logger.info(f"Total devices loaded from device_query_service: {len(nautobot_devices)}")
        ...
        logger.info(f"Nautobot IP lookup keys: {list(nautobot_lookup.keys())[:10]}")  # First 10 IPs
        ...
        for i, csv_device in enumerate(csv_devices):
            ...
            logger.info(f"Comparing CSV device: {csv_name} with IP {csv_ip} (cleaned: {csv_ip_clean})")
