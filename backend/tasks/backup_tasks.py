...
def finalize_backup_task(
    ...
    logger.info("Total devices processed: %s", len(device_results))
    logger.info("Successful backups: %s", len(backed_up_devices))
    logger.info("Failed backups: %s", len(failed_devices))
...
def backup_devices_task(
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Inventory devices: %s", len(inventory) if inventory else 0)
        logger.info("Config repository ID: %s", config_repository_id)
        ...
        logger.info("-" * 80)
        logger.info("STEP 3: BACKING UP %s DEVICES", len(inventory))
        ...
        logger.info("=" * 80)
        logger.info("Total devices: %s", total_devices)
        logger.info("Successful backups: %s", len(backed_up_devices))
        logger.info("Failed backups: %s", len(failed_devices))
