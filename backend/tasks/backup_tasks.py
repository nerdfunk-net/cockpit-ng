...
def backup_devices_task(
    ...
    try:
        ...
        from services.git_utils import repo_path, add_auth_to_url, set_ssl_env, resolve_git_credentials
        ...
        if not repository:
            logger.error(f"ERROR: Repository {config_repository_id} not found in database")
            return {
                'success': False,
        ...
        git_username, git_token = resolve_git_credentials(dict(repository))
        ...
        try:
            ...
            existing_url = normalize_git_url(git_repo.remotes.origin.url if git_repo.remotes else "")
            expected_url = normalize_git_url(repository.url)

            logger.info(f"Verifying repository URL...")
            logger.info(f"  - Expected: {expected_url}")
            logger.info(f"  - Actual:   {existing_url}")

            if existing_url != expected_url:
                logger.warning(f"⚠ Repository URL mismatch! Will remove and reclone.")
                git_repo.close()
            ...
            with set_ssl_env(dict(repository)):
                try:
                    ...
                    with git_repo.config_writer() as config:
                        config.set_value(f'remote "origin"', 'url', pull_url)
                    ...
                    logger.info(f"✓ Successfully recloned repository")

        self.update_state(
            state='PROGRESS',
            meta={'current': 20, 'total': 100, 'status': f'Backing up {len(inventory)} devices...'}
        )
        ...
        for idx, device_id in enumerate(inventory, 1):
            ...
            try:
                ...
                if not primary_ip:
                    logger.error(f"[{idx}] ✗ Device has no primary IP address - cannot connect")
                ...
                logger.info(f"[{idx}] Connecting to {device_name} ({primary_ip}) via SSH...")
                ...
                if not result['success']:
                    logger.error(f"[{idx}] ✗ SSH connection or command execution failed")
                    logger.error(f"[{idx}] Error: {result.get('error')}")
                    device_backup_info['error'] = result.get('error', 'SSH connection failed')
                    failed_devices.append(device_backup_info)
                    continue
                ...
                elif "Building configuration" in output or "Current configuration" in output:
                    # Fallback: try to find configuration markers
                    running_config = output.strip()
                    logger.info(f"[{idx}] Using entire output as running config (no split marker found)")
                else:
                    # Last resort: use entire output
                    running_config = output.strip()
                    logger.info(f"[{idx}] Using entire output as running config (no markers found)")
                ...
                if running_config:
                    ...
                    logger.info(f"[{idx}] ✓ Running config: {len(running_config)} bytes")
                else:
                ...
                if startup_config:
                    ...
                    logger.warning(f"[{idx}] ⚠ Startup config is empty or not retrieved")
                ...
                backed_up_devices.append({
                    ...
                    'startup_config_file': str(startup_file.relative_to(repo_dir)) if startup_config else None,
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 90, 'total': 100, 'status': 'Committing changes to Git...'}
        )
        ...
        try:
            if backed_up_devices:
                logger.info(f"Adding files to Git staging area...")
                git_repo.git.add('.')
                ...
                logger.info(f"Pushing to remote repository...")
                with set_ssl_env(dict(repository)):
                    origin = git_repo.remotes.origin
                    push_url = add_auth_to_url(repository.url, git_username, git_token)

                    with git_repo.config_writer() as config:
                        config.set_value(f'remote "origin"', 'url', push_url)

                    push_info = origin.push(repository.branch or 'main')
                    git_commit_status['pushed'] = True
                    logger.info(f"✓ Successfully pushed to {repository.branch or 'main'}")
...
def map_platform_to_netmiko(platform: str) -> str:
    ...
    if not platform:
        logger.warning(f"Platform is None or empty, defaulting to cisco_ios")
        return 'cisco_ios'
