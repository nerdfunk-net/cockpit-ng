...
def update_ip_addresses_from_csv_task(
    ...
    try:
        ...
        logger.info("Total IP addresses to process: %s", total_ip_addresses)
        ...
        logger.info("✓ Required column '%s' found", address_col)
        ...
        for idx, row in enumerate(rows, 1):
            ...
            try:
                ...
                if not address_value:
                    logger.warning("Row %s: Empty address value, skipping", idx)
                    skipped.append(
                        {
                            "row": idx,
                ...
                if ignore_uuid or not csv_uuid:
                    ...
                    logger.info("✓ Found IP address with UUID: %s", ip_address_uuid)
                else:
                    ...
                    try:
                        ...
                        if not existing_ip_address:
                            ...
                            continue
                        logger.info("✓ Verified IP address exists: %s", ip_address_uuid)
                    except Exception as e:
                        logger.error("Failed to verify IP address UUID: %s", e)
                        failures.append(
                            {
                                "row": idx,
                                "address": address_value,
...
async def _find_ip_address_by_address_and_namespace_graphql(
    ...
    try:
        ...
        logger.debug("Found IP address: %s", ip_address_data)

        return ip_address_uuid, ip_address_data

    except Exception as e:
        logger.error("Error finding IP address via GraphQL: %s", e, exc_info=True)
        return None, None
...
async def _update_ip_address(
    ...
    try:
        ...
        logger.info("[API CALL] ✓ Update successful for IP address %s", ip_address_uuid)
        return {"success": True}
