...
delegates the CIDR scan + port-scan + upsert loop to
ServerAnsibleOperationsService.run_open_ports_prefix_scan.
...
def execute_get_open_ports(
    ...
    config, error = load_prefix_scan_config(
        ...
        template,
        credential_id,
        agent_key="open_ports_agent_id",
        prefixes_key="open_ports_prefixes",
        auth_type_key="open_ports_auth_type",
        user_key="open_ports_ansible_user",
    )
    if error:
        return error
    ...
    try:
        ...
        return ops.run_open_ports_prefix_scan(
            config,
