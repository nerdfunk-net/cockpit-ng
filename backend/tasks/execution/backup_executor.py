...
def execute_backup(
    ...
    try:
        ...
        if job_parameters:
            ...
            logger.info(f"Config repository ID from job_parameters: {config_repository_id}")
        ...
        if git_repo is None:
            logger.info(f"CLONING repository from {repository.get('url')} to {repo_dir}")
            ...
            with set_ssl_env(dict(repository)):
                try:
                    ...
                    pull_url = add_auth_to_url(repository.get('url'), git_username, git_token)
        ...
        task_context.update_state(
            state='PROGRESS',
            meta={'current': 20, 'total': 100, 'status': f'Backing up {len(target_devices)} devices...'}
        )
        ...
        try:
            if backed_up_devices:
                ...
                with set_ssl_env(dict(repository)):
                    origin = git_repo.remotes.origin
                    push_url = add_auth_to_url(repository.get('url'), git_username, git_token)
