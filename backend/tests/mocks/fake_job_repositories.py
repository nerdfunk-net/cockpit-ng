...
class FakeJobTemplateRepository:
    ...
    def get_by_name(self, name: str, user_id: Optional[int] = None) -> Optional[_FakeJobTemplate]:
    ...
    def get_user_templates(self, user_id: int, job_type: Optional[str] = None) -> List[_FakeJobTemplate]:
        results = [t for t in self._templates.values() if t.is_global or t.user_id == user_id]
        if job_type is not None:
            results = [t for t in results if t.job_type == job_type]
        return sorted(results, key=lambda t: t.name)

    def get_global_templates(self, job_type: Optional[str] = None) -> List[_FakeJobTemplate]:
        results = [t for t in self._templates.values() if t.is_global]
        if job_type is not None:
            results = [t for t in results if t.job_type == job_type]
        return sorted(results, key=lambda t: t.name)

    def get_by_type(self, job_type: str, user_id: Optional[int] = None) -> List[_FakeJobTemplate]:
    ...
    def check_name_exists(self, name: str, user_id: Optional[int] = None, exclude_id: Optional[int] = None) -> bool:
        for tmpl in self._templates.values():
            if tmpl.name != name:
                continue
...
class FakeJobScheduleRepository:
    ...
    def get_user_schedules(self, user_id: int, is_active: Optional[bool] = None) -> List[_FakeJobSchedule]:
        results = [s for s in self._schedules.values() if s.is_global or s.user_id == user_id]
        if is_active is not None:
            results = [s for s in results if s.is_active == is_active]
        return sorted(results, key=lambda s: s.created_at, reverse=True)

    def get_global_schedules(self, is_active: Optional[bool] = None) -> List[_FakeJobSchedule]:
...
class FakeJobRunRepository:
    ...
    def get_by_celery_task_ids(self, celery_task_ids: List[str]) -> List[Dict[str, Any]]:
        return [r for r in self._runs.values() if r.get("celery_task_id") in celery_task_ids]
    ...
    def get_runs_since(
        ...
        results = [r for r in self._runs.values() if r.get("queued_at") and r["queued_at"] >= since]
        if status:
            results = [r for r in results if r.get("status") == status]
        if job_type:
            results = [r for r in results if r.get("job_type") == job_type]
    ...
    def aggregate_status_counts(self) -> Dict[str, int]:
        ...
        completed = sum(1 for r in self._runs.values() if r.get("status") == "completed")
    ...
    def cleanup_old_runs(self, days: int = 30) -> int:
        ...
        cutoff = datetime.now(timezone.utc) - timedelta(days=days)
        to_delete = [rid for rid, r in self._runs.items() if r.get("queued_at") and r["queued_at"] < cutoff]
    ...
    def cleanup_old_runs_hours(self, hours: int = 24) -> int:
        ...
        to_delete = [
            ...
            if r.get("queued_at") and r["queued_at"] < cutoff and r.get("status") in terminal
        ]
        for rid in to_delete:
            del self._runs[rid]
        return len(to_delete)
    ...
    def get_all_by_type_and_statuses(self, job_type: str, statuses: List[str]) -> List[Dict[str, Any]]:
        return [r for r in self._runs.values() if r.get("job_type") == job_type and r.get("status") in statuses]
