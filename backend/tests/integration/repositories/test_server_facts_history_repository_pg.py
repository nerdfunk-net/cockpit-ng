"""PostgreSQL integration tests for ``ServerFactsHistoryRepository``.
...
Run::
    ...
    pytest tests/integration/repositories/test_server_facts_history_repository_pg.py -v --no-cov
"""
...
_FACTS_V1 = {"ansible_hostname": "web01", "version": 1}
_FACTS_V2 = {"ansible_hostname": "web01", "version": 2}


@pytest.mark.integration
class TestServerFactsHistoryRepositoryPg:
    def test_get_by_server_id_orders_newest_first(
        self, server_facts_history_repository_pg
    ) -> None:
        servers_repo, history_repo = server_facts_history_repository_pg
        server = servers_repo.create(hostname="web01")
        history_repo.create(
            server_id=server.id, ansible_facts=_FACTS_V1, content_hash="hash1"
        )
        history_repo.create(
            server_id=server.id, ansible_facts=_FACTS_V2, content_hash="hash2"
        )
    ...
    def test_get_by_server_id_defers_ansible_facts(
        self, server_facts_history_repository_pg
    ) -> None:
        """List query does not load the large ``ansible_facts`` JSONB column."""
        servers_repo, history_repo = server_facts_history_repository_pg
        server = servers_repo.create(hostname="web01")
        history_repo.create(
            server_id=server.id, ansible_facts=_FACTS_V1, content_hash="hash1"
    ...
    def test_get_by_server_id_scoped_to_server(
        ...
        history_repo.create(
            server_id=server_a.id, ansible_facts=_FACTS_V1, content_hash="hash-a"
        )
        history_repo.create(
            server_id=server_b.id, ansible_facts=_FACTS_V2, content_hash="hash-b"
        )
    ...
    def test_get_by_id_scoped_returns_full_row(
        ...
        assert row.ansible_facts == _FACTS_V1
    ...
    def test_get_by_id_scoped_rejects_wrong_server(
        ...
        server_b = servers_repo.create(hostname="b")
        entry = history_repo.create(
            server_id=server_a.id, ansible_facts=_FACTS_V1, content_hash="hash1"
        )
