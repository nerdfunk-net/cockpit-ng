"""Unit tests for tasks/execution/get_server_facts_executor.py."""
...
from tasks.execution.get_server_facts_executor import execute_get_server_facts
...
def _call(job_parameters=None, template=None, credential_id=5):
    return execute_get_server_facts(
        schedule_id=None,
...
def test_invalid_auth_type_returns_error() -> None:
    result = _call(
        job_parameters={
            ...
            "facts_auth_type": "bogus",
        },
    )
    assert result["success"] is False
    assert "Invalid facts_auth_type" in result["error"]
                ...
                with patch(_PATCH_DB, return_value=_db_mock()):
                    with patch(_PATCH_SERVERS, return_value=servers_service):
                        with patch(_PATCH_AGENT) as agent_cls:
                            ...
                            agent_cls.return_value.send_ansible_get_facts.return_value = {
                                "status": "success",
...
def test_ssh_key_mode_success_skips_credential_lookup() -> None:
    ...
    call_kwargs = agent_cls.return_value.send_ansible_get_facts.call_args.kwargs
...
@pytest.mark.unit
def test_ssh_key_passphrase_mode_resolves_credential_but_does_not_persist_it() -> None:
    servers_service = MagicMock()
    servers_service.get_by_hostname.return_value = None
    servers_service.create.return_value = MagicMock(id=1)
    ...
    with patch(_PATCH_EXPAND, return_value=["10.0.0.1"]):
        with patch(_PATCH_FPING, return_value={"10.0.0.1"}):
            with patch(_PATCH_CREDENTIALS, return_value=_credentials_mock("keyuser")):
                with patch(_PATCH_DB, return_value=_db_mock()):
                    with patch(_PATCH_SERVERS, return_value=servers_service):
                        with patch(_PATCH_AGENT) as agent_cls:
                            agent_cls.return_value.check_agent_online.return_value = (
                            ...
                            result = _call(
                                job_parameters={
                                    "agent_id": "agent-1",
                                    "prefixes": ["10.0.0.0/30"],
                                    "facts_auth_type": "ssh_key_passphrase",
                                },
                            )
    ...
    # credential_id must not be persisted when use_sshkey is True (model constraint)
    assert created_creds.credential_id is None
                ...
                with patch(_PATCH_DB, return_value=_db_mock()):
                    with patch(_PATCH_SERVERS, return_value=servers_service):
                        with patch(_PATCH_AGENT) as agent_cls:
                            ...
                            agent_cls.return_value.send_ansible_get_facts.side_effect = _side_effect
                            result = _call(
                                job_parameters={
                                    "agent_id": "agent-1",
