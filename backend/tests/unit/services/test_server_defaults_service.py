"""Unit tests for services/settings/server_defaults_service.py."""
...
_PATCH_PROFILE_REPO = "services.settings.server_defaults_service.ProfileRepository"

_DEFAULT_SERVER_DEFAULTS = ServerDefaults()


def _make_service() -> ServerDefaultsService:
    return ServerDefaultsService(default=_DEFAULT_SERVER_DEFAULTS)
...
def test_get_returns_db_values_when_exist():
    ...
    mock_repo.get_by_built_in_key.assert_called_once_with("server")
...
def test_update_creates_when_no_existing():
    ...
    assert mock_repo.create.call_args.kwargs["built_in_key"] == "server"
    assert mock_repo.create.call_args.kwargs["name"] == "Server"
