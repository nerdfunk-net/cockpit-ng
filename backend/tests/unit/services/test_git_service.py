...
def test_get_repository_status_when_path_missing() -> None:
    svc = _service()
    with patch(
        "services.git.service.get_repo_path", return_value=Path("/nonexistent/repo")
    ):
        with patch("services.git.service.os.path.exists", return_value=False):
...
def test_get_repository_status_populates_git_fields() -> None:
    ...
    with patch("services.git.service.get_repo_path", return_value=Path("/tmp/repo")):
        with patch("services.git.service.os.path.exists", return_value=True):
            with patch("services.git.service.Repo", return_value=mock_repo):
                with patch(
...
def test_sync_repository_pull_success() -> None:
    ...
    mock_origin.url = _REPO["url"]
    mock_origin.pull.return_value = [MagicMock()]
    mock_repo.remotes.origin = mock_origin

    with patch("services.git.service.get_repo_path", return_value=Path("/tmp/repo")):
        with patch("services.git.service.os.makedirs"):
            with patch("services.git.service.os.path.exists", return_value=True):
                with patch(
                    "services.git.service.os.path.isdir",
                    side_effect=lambda p: str(p).endswith(".git"),
                ):
                    with patch("services.git.service.set_ssl_env"):
                        with patch("services.git.service.Repo", return_value=mock_repo):
                            with patch.object(Path, "mkdir", return_value=None):
...
def test_sync_repository_clone_auth_error_message() -> None:
    ...
    with patch(
        "services.git.service.get_repo_path", return_value=Path("/tmp/new-repo")
    ):
        with patch("services.git.service.os.makedirs"):
            with patch("services.git.service.os.path.exists", return_value=False):
                with patch("services.git.service.os.path.isdir", return_value=False):
                    with patch("services.git.service.set_ssl_env"):
                        with patch(
                            "services.git.service.Repo.clone_from",
                            side_effect=gce,
                        ):
...
def test_remove_and_sync_clones_fresh() -> None:
    ...
    with patch("services.git.service.get_repo_path", return_value=Path("/tmp/repo")):
        with patch("services.git.service.os.path.exists", return_value=True):
            with patch("services.git.service.shutil.move"):
                with patch("services.git.service.set_ssl_env"):
                    with patch("services.git.service.Repo.clone_from"):
                        with patch.object(Path, "exists", return_value=False):
                            with patch.object(Path, "mkdir", return_value=None):
