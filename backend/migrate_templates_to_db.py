...
def migrate_templates_to_database():
    ...
    for template in templates:
        ...
        if template.source == "git":
            logger.info(f"  Skipping Git template")
            stats["already_in_db"] += 1
            continue

        # Check if content already in database
        if template.content and len(template.content.strip()) > 0:
            logger.info(f"  ✓ Content already in database ({len(template.content)} chars)")
            stats["already_in_db"] += 1
            continue
        ...
        if content:
            ...
            logger.warning(f"  ! No content found (neither in DB nor file)")
            stats["no_content"] += 1
