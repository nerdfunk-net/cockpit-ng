...
class JobScheduleBase(BaseModel):
    ...
    job_identifier: str = Field(..., description="Unique identifier for this schedule (user-defined)")
    job_template_id: int = Field(..., description="ID of the job template to use")
    schedule_type: Literal["now", "interval", "hourly", "daily", "weekly", "monthly", "custom"] = Field(
...
class JobScheduleResponse(JobScheduleBase):
    ...
    id: int
    job_template_id: Optional[int] = Field(None, description="ID of the job template to use")
    ...
    template_name: Optional[str] = Field(None, description="Name of the associated job template")
    template_job_type: Optional[str] = Field(None, description="Job type from the template")
...
class JobRunBase(BaseModel):
    ...
    job_schedule_id: Optional[int] = Field(None, description="ID of the schedule that triggered this run")
    ...
    triggered_by: Literal["schedule", "manual"] = Field("schedule", description="How the job was triggered")
    target_devices: Optional[list[str]] = Field(None, description="List of target device names")
    executed_by: Optional[str] = Field(None, description="Username who triggered manual run")
...
class JobRunResponse(BaseModel):
    ...
    started_at: Optional[datetime] = None
    completed_at: Optional[datetime] = None
    duration_seconds: Optional[float] = Field(None, description="Duration in seconds (computed)")
    error_message: Optional[str] = None
    result: Optional[dict] = None
    target_devices: Optional[list[str]] = None
    executed_by: Optional[str] = None
    # Additional fields from relationships
    schedule_name: Optional[str] = Field(None, description="Name of the schedule (if still exists)")
    template_name: Optional[str] = Field(None, description="Name of the template (if still exists)")
