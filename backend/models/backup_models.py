...
class DeviceBackupInfo(BaseModel):
    ...
    ssh_connection_success: bool = Field(
        default=False,
        description="Whether SSH connection to device succeeded"
    )
    running_config_success: bool = Field(
        default=False,
        description="Whether running config was successfully retrieved"
    )
    startup_config_success: bool = Field(
        default=False,
        description="Whether startup config was successfully retrieved"
    )
    
    # Configuration details
    running_config_bytes: int = Field(
        default=0,
        description="Size of running configuration in bytes"
    )
    startup_config_bytes: int = Field(
        default=0,
        description="Size of startup configuration in bytes"
    )
    
    # File paths (relative to repository root)
    running_config_file: Optional[str] = Field(
        None,
        description="Path to saved running config file"
    )
    startup_config_file: Optional[str] = Field(
        None,
        description="Path to saved startup config file"
    )
...
class GitStatus(BaseModel):
    ...
    repository_existed: bool = Field(
        default=False,
        description="Whether repository existed locally before operation"
    )
    ...
    branch: Optional[str] = Field(
        None,
        description="Git branch used for backup"
    )
...
class CredentialInfo(BaseModel):
    ...
    credential_id: Optional[int] = Field(
        None,
        description="Credential ID from database"
    )
    ...
    username: Optional[str] = Field(
        None,
        description="SSH username used for device authentication"
    )
...
class TimestampUpdateStatus(BaseModel):
    ...
    enabled: bool = Field(
        default=False,
        description="Whether timestamp updates were enabled"
    )
    custom_field_name: Optional[str] = Field(
        None,
        description="Name of custom field to update"
    )
    updated_count: int = Field(
        default=0,
        description="Number of devices successfully updated"
    )
    ...
    errors: list[str] = Field(
        ...
        description="List of error messages from failed updates"
    )
...
class BackupResult(BaseModel):
    ...
    success: bool = Field(
        ...,
        description="Overall success status of backup operation"
    )
    backed_up_count: int = Field(
        default=0,
        description="Number of devices successfully backed up"
    )
    failed_count: int = Field(
        default=0,
    ...
    backed_up_devices: list[dict] = Field(
        default_factory=list,
        description="List of successfully backed up devices"
    )
    failed_devices: list[dict] = Field(
        default_factory=list,
        description="List of failed device backups"
    )
    git_status: GitStatus = Field(
        default_factory=GitStatus,
        description="Git repository status"
    )
    git_commit_status: GitCommitStatus = Field(
        default_factory=GitCommitStatus,
        description="Git commit/push status"
    )
    credential_info: CredentialInfo = Field(
        default_factory=CredentialInfo,
        description="Credential information used"
    )
    ...
    commit_date: Optional[str] = Field(
        None,
        description="Backup commit timestamp"
    )
    error: Optional[str] = Field(
        None,
        description="Overall error message if backup failed"
    )
    
    # Backward compatibility fields
    devices_backed_up: Optional[int] = Field(
        None,
        description="Alias for backed_up_count (UI compatibility)"
    )
    devices_failed: Optional[int] = Field(
        None,
        description="Alias for failed_count (UI compatibility)"
    )
