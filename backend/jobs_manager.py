...
def create_job_schedule(
    ...
    logger.info(f"Created job schedule: {job_identifier} (ID: {schedule.id}, next_run: {initial_next_run})")
    ...
    for schedule in schedules:
        if schedule.get('next_run') is None:
            ...
            if next_run:
                ...
                logger.info(f"Initialized next_run for schedule {schedule['id']}: {next_run.isoformat()}")
                initialized += 1
    
    return {
        'initialized_count': initialized,
        'total_active': len(schedules)
    }
    ...
    if schedule_type == 'now':
        ...
        try:
            ...
            if base_time.month == 12:
                next_run = base_time.replace(year=base_time.year + 1, month=1, day=base_time.day,
                                            hour=hour, minute=minute, second=0, microsecond=0)
            else:
                next_run = base_time.replace(month=base_time.month + 1, hour=hour, minute=minute,
...
def calculate_and_update_next_run(job_id: int) -> Optional[Dict[str, Any]]:
    ...
    if not schedule:
        logger.warning(f"Schedule {job_id} not found for next_run calculation")
        return None
    ...
    logger.debug(f"Updated schedule {job_id}: last_run={now.isoformat()}, next_run={next_run.isoformat() if next_run else None}")
