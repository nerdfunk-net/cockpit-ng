...
def migrate_permission_names():
    ...
    for old_name, new_name in renames:
        ...
        perms_to_update = [
            p for p in all_perms if p["resource"] == old_name
        ]

        for perm in perms_to_update:
            print(f"  Renaming: {old_name}:{perm['action']} -> {new_name}:{perm['action']}")
    ...
    print("\nMigration complete!")
    print("\nUpdated permissions:")
    settings_perms = [p for p in rbac.list_permissions() if p["resource"].startswith("settings.")]
