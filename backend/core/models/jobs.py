...
class JobTemplate(Base):
    ...
    timestamp_custom_field_name = Column(String(255))  # Name of the Nautobot custom field to write the timestamp to
    ...
    scan_timeout_ms = Column(Integer)  # Timeout in milliseconds for network operations (scan_prefixes type)
    scan_retries = Column(Integer)  # Number of retry attempts for failed operations (scan_prefixes type)
    scan_interval_ms = Column(Integer)  # Interval in milliseconds between scan operations (scan_prefixes type)
    scan_custom_field_name = Column(String(255))  # Name of custom field for prefix selection (scan_prefixes type)
    scan_custom_field_value = Column(String(255))  # Value of custom field to filter prefixes (scan_prefixes type)
    ...
    scan_max_ips = Column(Integer)  # Maximum number of IPs to scan per job (scan_prefixes type)
    parallel_tasks = Column(
        Integer, nullable=False, default=1
    )  # Number of parallel tasks for backup execution (backup type)
    deploy_template_id = Column(Integer, nullable=True)  # ID of the agent template to deploy (deploy_agent type)
    deploy_agent_id = Column(String(255), nullable=True)  # ID of the agent to deploy to (deploy_agent type)
    deploy_path = Column(String(500), nullable=True)  # File path for deployment (deploy_agent type)
    deploy_custom_variables = Column(Text, nullable=True)  # JSON string of user variable overrides (deploy_agent type)
    ...
    set_primary_ip_strategy = Column(String(50), nullable=True)  # "ip_reachable" or "interface_name"
    set_primary_ip_agent_id = Column(String(255), nullable=True)  # cockpit agent ID used for reachability ping
    ...
    collect_hostname = Column(Boolean, nullable=False, default=True)  # Resolve hostnames via DNS (get_client_data type)
...
class JobSchedule(Base):
    ...
    template = relationship("JobTemplate", backref=backref("schedules", cascade="all, delete-orphan"))
