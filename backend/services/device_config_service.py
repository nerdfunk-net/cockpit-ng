...
class DeviceConfigService:
    ...
    def retrieve_device_configs(
        ...
        if not result["success"]:
            logger.error(
                f"{log_prefix} ✗ SSH connection or command execution failed"
            )
    ...
    def parse_config_output(
        ...
        if running_config:
            logger.info(
                f"{log_prefix} ✓ Running config: {len(running_config)} bytes"
            )
        else:
        ...
        if startup_config:
            logger.info(
                f"{log_prefix} ✓ Startup config: {len(startup_config)} bytes"
            )
        else:
            logger.info(
                f"{log_prefix} Startup config is empty or not retrieved"
            )
