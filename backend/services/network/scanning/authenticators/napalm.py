...
class NapalmAuthenticator:
    ...
    async def authenticate(self, ip: str, username: str, password: str) -> Optional[Dict[str, str]]:
        for driver_name in _CISCO_DRIVERS:
            try:
                result = await asyncio.to_thread(self._connect_get_facts, driver_name, ip, username, password)
                if result:
                    return result
    ...
    def _connect_get_facts(self, driver_name: str, ip: str, username: str, password: str) -> Optional[Dict[str, str]]:
        try:
