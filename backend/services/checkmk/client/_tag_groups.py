...
class _TagGroupsMixin:
    ...
    def create_host_tag_group(self, id: str, title: str, tags: List[Dict], topic: str = None, help: str = None) -> Dict:
        ...
        response = self._make_request("POST", "domain-types/host_tag_group/collections/all", json_data=json_data)
        return self._handle_response(response)
    ...
    def update_host_tag_group(
        ...
        response = self._make_request("PUT", f"objects/host_tag_group/{name}", json_data=json_data, etag=etag)
        return self._handle_response(response)

    def delete_host_tag_group(self, name: str, repair: bool = False, mode: str = None) -> bool:
        params = {"repair": repair}
        if mode is not None:
            params["mode"] = mode

        response = self._make_request("DELETE", f"objects/host_tag_group/{name}", params=params)
