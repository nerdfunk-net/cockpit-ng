...
class NB2CMKBackgroundService:
    ...
    async def _process_devices_diff(self, job_id: str) -> None:
        ...
        try:
            ...
            nb2cmk_db_service.update_job_progress(
                job_id, 0, total_devices, f"Starting comparison of {total_devices} devices"
            )
            ...
            for device in nautobot_devices:
                try:
                    ...
                    logger.debug(f"Job {job_id}: Processing device {device_name} ({device_id})")
                    ...
                    try:
                        ...
                        if http_exc.status_code == 404:
                            logger.debug(f"Job {job_id}: Device {device_name} not found in CheckMK")
                            device_info["checkmk_status"] = "missing"
                            device_info["diff"] = f"Host '{device_name}' not found in CheckMK"
                            device_info["normalized_config"] = {}
                            device_info["checkmk_config"] = None
                        else:
                            logger.warning(f"Job {job_id}: HTTP error comparing device {device_name}: {http_exc}")
                        ...
                        logger.warning(f"Job {job_id}: Error comparing device {device_name}: {e}")
                        device_info["checkmk_status"] = "error"
                    ...
                    logger.error(f"Job {job_id}: Error processing device {device.get('name', 'unknown')}: {e}")
                    processed_count += 1
            ...
            try:
                if 'processed_count' in locals() and 'total_devices' in locals():
                    nb2cmk_db_service.update_job_progress(
                        job_id, processed_count, total_devices, f"Failed after processing {processed_count} devices: {error_msg}"
                    )
            except:
