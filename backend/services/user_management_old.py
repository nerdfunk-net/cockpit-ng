    ...
    try:
        ...
        try:
            ...
            for cred in credentials:
                if cred['username'] == username and cred['name'] == f"{username} User Account":
...
def get_user_by_username(username: str) -> Optional[Dict[str, Any]]:
    ...
    with _get_conn() as conn:
        row = conn.execute(
            "SELECT * FROM user_management WHERE username = ?",
            (username,)
        ).fetchone()
...
def update_user(
    ...
    try:
        with _get_conn() as conn:
            ...
            conn.execute(
                f"UPDATE user_management SET {', '.join(updates)} WHERE id = ?",
                params
            )
            conn.commit()
...
def delete_user(user_id: int) -> bool:
    ...
    try:
        ...
        with _get_conn() as conn:
            conn.execute("DELETE FROM user_profiles WHERE username = ?", (user["username"],))
            conn.commit()
...
def bulk_update_permissions(user_ids: List[int], permissions: UserPermissions) -> Tuple[int, List[str]]:
