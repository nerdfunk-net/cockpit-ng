...
class OffboardingService:
    ...
    async def offboard_device(
        ...
        logger.info("DEBUG: Retrieved offboarding settings: %s", offboarding_settings is not None)
        ...
        if integration_mode == "remove":
            ...
            if offboarding_settings.get("clear_device_name", False):
                logger.info("DEBUG: clear_device_name is True - clearing device name for %s", device_id)
                try:
                    ...
                    results["errors"].append(f"Failed to clear device name: {exc.detail}")
                    results["success"] = False
                    logger.error("Failed to clear device name for %s: %s", device_id, exc.detail)
                except Exception as exc:
                    error_msg = f"Failed to clear device name for {device_id}: {str(exc)}"
        ...
        if request.remove_interface_ips:
            ...
            results["skipped_items"].append(
                "Interface IP removal was not requested"
            )
        ...
        if request.remove_from_checkmk:
            ...
            results["skipped_items"].append(
                "CheckMK removal was not requested"
            )
    ...
    async def _handle_set_offboarding_values(
        ...
        logger.info("DEBUG: _handle_set_offboarding_values called for device %s", device_id)
        logger.info("Applying custom fields offboarding settings to device %s", device_id)
        ...
        try:
            ...
            logger.info("DEBUG: Retrieved %d custom field definitions", len(custom_field_list) if isinstance(custom_field_list, list) else 0)
        except Exception as exc:
            logger.error("DEBUG: Failed to fetch custom field definitions: %s", str(exc))
            results["errors"].append(f"Failed to fetch custom field definitions: {str(exc)}")
            results["success"] = False
            return
        ...
        for field_def in custom_field_list:
            ...
            logger.info("DEBUG: Processing custom field '%s' with default value: %s", label, default_value)
            ...
            if remove_all_custom_fields:
                ...
                logger.info("DEBUG: remove_all_custom_fields=True - Setting field '%s' to default value: %s", label, default_value)
            # Otherwise, check if this field is configured in custom_field_settings
                ...
                logger.info("DEBUG: Found configuration for field '%s': %s", label, configured_value)

                if configured_value == "clear":
                    # Use the default value
                    custom_fields_update[label] = default_value
                    logger.info("DEBUG: Setting field '%s' to default value: %s", label, default_value)
                else:
                    # Use the configured value
                    custom_fields_update[label] = configured_value
                    logger.info("DEBUG: Setting field '%s' to configured value: %s", label, configured_value)

        # 4. Update the device with the custom_fields_update dict
        if not custom_fields_update:
            results["skipped_items"].append("No custom field updates specified in offboarding settings")
        ...
        try:
            ...
            results["removed_items"].append("Device custom fields updated for offboarding")
            logger.info("Applied offboarding custom field settings to device %s", device_id)
        except HTTPException as exc:
            results["errors"].append(exc.detail)
            results["success"] = False
    ...
    async def _remove_primary_ip(
        ...
        if not request.remove_primary_ip:
            results["skipped_items"].append(
                "Primary IP removal was not requested"
            )
        ...
        try:
            ...
            results["removed_items"].append(
                f"Primary IP: {primary_ip_address}"
            )
            ...
            error_msg = (
                f"Failed to remove primary IP {primary_ip_address}: {str(exc)}"
            )
            results["errors"].append(error_msg)
            logger.error(error_msg)
    ...
    async def _remove_from_checkmk(
        ...
        try:
            ...
            logger.info(
                "Successfully removed device %s from CheckMK", device_name
            )
        except HTTPException as exc:
            results["errors"].append(exc.detail)
            logger.error(
    ...
    async def _delete_ip_address(self, ip_id: str, device_id: str) -> Dict[str, Any]:
        try:
            ...
            raise self._translate_exception(
                exc, f"Failed to delete IP address {ip_id}"
            )
    ...
    async def _update_device(
        ...
        try:
            ...
            raise self._translate_exception(
                exc, f"Failed to update device {device_id}"
            )
    ...
    def _validate_offboarding_settings(
        ...
        if remove_all_fields or clear_device_name:
            logger.info("DEBUG: remove_all_custom_fields=%s or clear_device_name=%s - validation passed", remove_all_fields, clear_device_name)
            return True
    ...
    @staticmethod
    def _build_custom_field_payload(settings: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not settings:
            return None
