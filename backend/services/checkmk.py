...
class CheckMKService:
    ...
    async def test_connection(self, url: str, site: str, username: str, password: str, verify_ssl: bool = True) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if client.test_connection():
                ...
                return False, "Connection test failed. Please check your credentials and server configuration."
            ...
            if e.status_code == 401:
                ...
                return False, "CheckMK API not found. Please verify the server URL, site name, and that CheckMK is properly installed."
            else:
                return False, f"API error (HTTP {e.status_code}): {str(e)}"
        except ImportError:
            logger.error("CheckMK client not available")
            return False, "CheckMK client module not found. Please ensure the CheckMK client is properly installed."
