...
async def get_config_repositories(current_user: dict = Depends(verify_admin_token)):
    ...
    try:
        repositories = git_repo_manager.get_repositories(
            category="configs", active_only=True
        )
...
async def set_selected_repository(
    repository_id: int, current_user: dict = Depends(verify_admin_token)
):
    ...
    try:
        ...
        if repository["category"] != "configs":
            raise HTTPException(
                status_code=400,
                detail="Only repositories in 'configs' category can be selected",
            )

        if not repository["is_active"]:
            raise HTTPException(
                status_code=400, detail="Repository must be active to be selected"
            )
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to save selected repository"
            )

        return {
            "message": f"Repository '{repository['name']}' selected successfully",
                    ...
                    try:
                        ...
                        if (
                            ...
                            if len(commit_info) >= 5:
                                ...
                                status_info["last_commit_author"] = commit_info[
                                    3
                                ]  # Author name
                                status_info["last_commit_author_email"] = commit_info[
                                    4
                                ]  # Author email
                    except Exception as e:
...
async def sync_repositories(
    sync_request: GitSyncRequest, current_user: dict = Depends(verify_admin_token)
):
