...
async def get_device_normalized(
    ...
    try:
        ...
        try:
            ...
            checkmk_config_path = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "../config", "checkmk.yaml"
            )

            with open(checkmk_config_path, "r") as f:
                checkmk_config = yaml.safe_load(f)

            additional_attributes_config = checkmk_config.get(
                ...
                try:
                    ...
                    for ip_or_cidr, additional_attrs in by_ip_config.items():
                        try:
                            ...
                            logger.warning(
                                f"Error processing additional_attributes IP rule '{ip_or_cidr}': {str(e)}"
                            )
                            continue
            ...
            if mapping_config:
                for nautobot_field, checkmk_attribute in mapping_config.items():
                    try:
                        ...
                        value = None
                        logger.info(
                            f"Processing mapping: {nautobot_field} → {checkmk_attribute}"
                        )
                        ...
                        if "." in nautobot_field:
                            ...
                            for part in field_parts:
                                if (
                                    ...
                                    logger.info(
                                        f"Nested field '{nautobot_field}' failed at part '{part}' - not found or not dict"
                                    )
                                    current_data = None
                                    break
                            value = current_data
                        ...
                        logger.warning(
                            f"Error processing mapping '{nautobot_field}' → '{checkmk_attribute}': {str(e)}"
                        )
                        continue
            ...
            try:
                ...
                for cidr_network, site_value in by_ip_config.items():
                    try:
                        ...
                        logger.warning(
                            f"Invalid CIDR network in site config: {cidr_network}"
                        )
                        continue
            ...
            try:
                ...
                for cidr_network, folder_template in by_ip_config.items():
                    try:
                        ...
                        logger.warning(
                            f"Invalid CIDR network in config: {cidr_network}"
                        )
                        continue
    ...
    try:
        ...
        for device in nautobot_devices:
            ...
            try:
                ...
                if device_id:
                    comparison_result = await compare_device_config(
                        device_id, current_user
                    )
                    device_info["checkmk_status"] = comparison_result["result"]
                    device_info["diff"] = comparison_result.get("diff", "")
                    device_info["normalized_config"] = comparison_result.get(
                        "normalized_config", {}
                    )
                    device_info["checkmk_config"] = comparison_result.get(
                        "checkmk_config", {}
                    )
                ...
                if http_exc.status_code == 404:
                    ...
                    logger.warning(
                        f"HTTP error comparing device {device.get('name', 'unknown')}: {str(http_exc)}"
                    )
                    device_info["checkmk_status"] = "error"
            except Exception as e:
                logger.warning(
                    f"Error comparing device {device.get('name', 'unknown')}: {str(e)}"
                )
                device_info["checkmk_status"] = "error"
    ...
    try:
        ...
        try:
            ...
            if folder and folder != "/":
                ...
                try:
                    folder_created = await create_path(
                        folder, device_site, current_user
                    )
            ...
            if hasattr(e, "status_code"):
                logger.error(
                    f"  Status code: {getattr(e, 'status_code', 'No status code')}"
                )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_device_in_checkmk(
    ...
    try:
        ...
        current_folder_normalized = (
            current_folder.rstrip("/") if current_folder != "/" else "/"
        )
        ...
        if folder_changed:
            ...
            path_created = await create_path(
                new_folder_normalized, device_site, current_user
            )
...
async def create_path(folder_path: str, site_name: str, current_user: dict) -> bool:
    ...
    try:
        ...
        path_parts = (
            folder_path.lstrip("~").split("~")
            if folder_path.startswith("~")
            else folder_path.lstrip("/").split("/")
        )
        ...
        for i in range(len(path_parts)):
            ...
            try:
                ...
                if "already exists" in str(e).lower() or "400" in str(e):
                    ...
                    logger.error(
                        f"CheckMK API error creating folder '{folder_name}': {str(e)}"
                    )
                    return False
            except Exception as e:
