...
async def _get_device_data_from_cache_or_nautobot(device_name: str) -> Optional[dict]:
    ...
    try:
        ...
        if "errors" in result:
            logger.warning(
                f"GraphQL errors fetching device {device_name}: {result['errors']}"
            )
            return None
        ...
        logger.debug(
            f"Found device data for {device_name}: role={device.get('role', {}).get('name')}, location={device.get('location', {}).get('name')}"
        )
        return device
...
async def _enrich_device_results_with_nautobot_data(
    ...
    for result in device_results:
        ...
        if device_data:
            ...
            enriched_result.update(
                {
                    ...
                    "device_status": device_data.get(
                        "status"
                    ),  # Use device_status to avoid conflict with job result status
                }
            )
            logger.debug(
                f"Enriched device result for {device_name} with role, location, and status"
            )
        else:
            ...
            logger.debug(
                f"Could not enrich device result for {device_name} - using placeholders"
            )
...
class ParallelJobRequest(BaseModel):
    devices: List[str] = Field(
        default=[],
        max_items=50,
        description="List of devices to process (empty = all devices)",
    )
    max_concurrent: int = Field(
        default=3, ge=1, le=10, description="Maximum concurrent device processing"
    )
...
async def get_job_details(
    ...
    try:
        ...
        # Enrich device results with additional data from Nautobot
        logger.info(
            f"=== STARTING ENRICHMENT: {len(basic_device_results)} device results ==="
        )
        logger.info(
            f"Basic device results sample: {basic_device_results[0] if basic_device_results else 'No results'}"
        )
        enriched_device_results = await _enrich_device_results_with_nautobot_data(
            basic_device_results
        )
        logger.info("=== ENRICHMENT COMPLETED ===")
        logger.info(
            f"Enriched device results sample: {enriched_device_results[0] if enriched_device_results else 'No results'}"
        )
        logger.info(
            f"Enhanced fields added: {set(enriched_device_results[0].keys()) - set(basic_device_results[0].keys()) if enriched_device_results and basic_device_results else 'No comparison'}"
        )
        ...
        job_dict = {
            ...
            if job_data.get("progress_total", 0) > 0
            else None,
            "result_summary": job_data["result_summary"],
            "error_message": job_data["error_message"],
            "metadata": job_data["metadata"],
...
async def _execute_network_scan(
    job_id: str, cidr: str, request: NetworkScanRequest, username: str
):
    ...
    try:
        ...
        if result.error_message:
            ...
            job_db_service.update_job_status(
                job_id, JobStatus.FAILED, error_message=result.error_message
            )
            logger.error(f"Network scan job {job_id} failed: {result.error_message}")
        else:
            # Job completed successfully
            job_db_service.update_job_status(job_id, JobStatus.COMPLETED)
            logger.info(
        ...
        job_db_service.update_job_status(job_id, JobStatus.FAILED, error_message=str(e))
