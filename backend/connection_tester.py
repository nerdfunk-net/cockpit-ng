    ...
    async def test_git_connection(settings: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                if branch and branch != "main":
                    ...
                    if f"refs/heads/{branch}" not in branches:
                        ...
                        if available_branches:
                            return (
                                False,
                                f"Branch '{branch}' not found. Available branches: {', '.join(available_branches[:5])}",
                            )
                        else:
                            return (
                                False,
                                f"Branch '{branch}' not found and no branches detected.",
                            )

                return (
                    True,
                    f"Connection successful! Repository is accessible and branch '{branch}' exists.",
                )
                ...
                return (
                    False,
                    "Git command not found. Please ensure Git is installed on the server.",
                )
    ...
    async def test_all_connections(
        ...
        try:
            (
                nautobot_success,
                nautobot_msg,
            ) = await ConnectionTester.test_nautobot_connection(nautobot_settings)
            results["nautobot"] = {"success": nautobot_success, "message": nautobot_msg}
        except Exception as e:
            results["nautobot"] = {
                "success": False,
